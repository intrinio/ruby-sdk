...
module Intrinio
  class SecurityApi
    ...
    def get_security_interval_prices_with_http_info(identifier, interval_size, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'source'] && !['realtime', 'delayed', 'nasdaq_basic', 'nasdaq_basic_filtered', 'nasdaq_basic_last_sale', 'cboe_one'].include?(opts[:'source'])
        fail ArgumentError, 'invalid value for "source", must be one of realtime, delayed, nasdaq_basic, nasdaq_basic_filtered, nasdaq_basic_last_sale, cboe_one'
      end
