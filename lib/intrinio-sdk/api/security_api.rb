...
module Intrinio
  class SecurityApi
    ...
    # @option opts [String] :interval_size The interval for which to return stock prices (default to 1d)
    # @option opts [Integer] :page_size The number of results to return (default to 100)
    # @option opts [String] :next_page Gets the next page of data from a previous API call
    # @return [ApiResponseSecurityIntervalPrices]
    def get_security_interval_prices(identifier, opts = {})
      data, _status_code, _headers = get_security_interval_prices_with_http_info(identifier, opts)
      return data
    end
    ...
    def get_security_interval_prices_with_http_info(identifier, opts = {})
      if @api_client.config.debugging
      ...
      query_params[:'interval_size'] = opts[:'interval_size'] if !opts[:'interval_size'].nil?
      query_params[:'page_size'] = opts[:'page_size'] if !opts[:'page_size'].nil?
      query_params[:'next_page'] = opts[:'next_page'] if !opts[:'next_page'].nil?
    ...
    def get_security_realtime_price_with_http_info(identifier, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'source'] && !['iex', 'bats', 'bats_delayed', 'intrinio_mx'].include?(opts[:'source'])
        fail ArgumentError, 'invalid value for "source", must be one of iex, bats, bats_delayed, intrinio_mx'
      end
      # resource path
