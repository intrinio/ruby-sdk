...
module Intrinio
  class OptionsApi
    ...
    def get_security_snapshots_with_http_info(opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug "Calling API: OptionsApi.get_security_snapshots ..."
      end
      if @api_client.config.client_side_validation && opts[:'source'] && !['realtime', 'delayed'].include?(opts[:'source'])
        fail ArgumentError, 'invalid value for "source", must be one of realtime, delayed'
      end
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: OptionsApi#get_security_snapshots\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
